...
const BookingFormModal = ({ showBookingModal, selectedRoom, onClose }) => {
  ...
  return (
    <>
      {showBookingModal && selectedRoom && (
        <div className={`fixed inset-0 ${isDarkMode ? 'bg-black bg-opacity-70' : 'bg-gray-500 bg-opacity-50'} flex items-center justify-center z-50`}>
          <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'} rounded-xl shadow-2xl w-11/12 max-w-4xl flex overflow-hidden`}>
            <div className={`${isDarkMode ? 'bg-gradient-to-br from-gray-900 to-blue-900' : 'bg-gradient-to-br from-gray-200 to-blue-200'} w-7/12 p-6`}>
              <h3 className={`text-2xl font-semibold ${isDarkMode ? 'text-blue-300' : 'text-blue-600'} mb-4`}>
              ...
              {selectedRoom.images && selectedRoom.images.length > 0 ? (
                ...
                <div className={`${isDarkMode ? 'bg-gray-800 text-gray-600 shadow-gray-900' : 'bg-gray-300 text-gray-500 shadow-gray-400'} w-full h-48 rounded-lg flex items-center justify-center shadow-inner mb-4`}>
                  No Images Available
                </div>
              )}
            ...
            <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'} w-5/12 p-6`}>
              <div className="flex justify-between items-center mb-4">
                <h3 className={`text-xl font-semibold ${isDarkMode ? 'text-blue-300' : 'text-blue-600'}`}>Book This Room</h3>
                <button
                  onClick={() => {
                    onClose();
                    setCouponError("");
