...
//                   type="submit"
//                   disabled={loading || !!couponError}
//                   className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white py-3 rounded-lg transition-all duration-200 shadow-lg hover:shadow-blue-900/50 font-medium flex items-center justify-center disabled:opacity-50GrossMargin disabled:cursor-not-allowed"
//                 >
//                   {loading ? "Submitting..." : "Submit Booking"}
//                 </button>
//               </form>
...
// import BookingsModal from "./components/BookingshistoryModal";
...
//       setRooms(activeRooms);
//       setCurrentImageIndex(activeRooms.reduce((acc, room) => {
//         acc[room._id] = 0;
//         return acc;
//       }, {}));
//     } catch (err) {
...
//         headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
//       });
//       setCoupons(response.data.filter(
...
//       if (coupon) {
//         const now = new Date();
//         if (now >= new Date(coupon.startDate) && now <= new Date(coupon.endDate) && coupon.isActive) {
//           discount = coupon.discount;
//         } else {
//           couponErrorMsg = "Selected coupon is not valid or has expired.";
//         }
...
//           onToggleHotelDetails={(hotelId) => {
//             scrollPositionRef.current = window.scrollY;
//             setExpandedHotels((prev) => ({ ...prev, [hotelId]: !prev[hotelId] }));
//             if (!expandedHotels[hotelId]) fetchRooms(hotelId);
//           }}
//           onImageChange={(roomId, direction, totalImages) => {
//             setCurrentImageIndex((prev) => ({
//               ...prev,
//               [roomId]:
//                 direction === "prev"
...
//           loading={loading}
//           currentImageIndex={currentImageIndex[selectedRoom._id] || 0}
//           onClose={() => {
//             setShowBookingModal(false);
//             setCouponError("");
//           }}
...
//               [selectedRoom._id]:
//                 direction === "prev"
//                   ? prev[selectedRoom._id] === 0
//                     ? selectedRoom.images.length - 1
//                     : prev[selectedRoom._id] - 1
//                   : prev[selectedRoom._id] === selectedRoom.images.length - 1
//                   ? 0
//                   : prev[selectedRoom._id] + 1,
//             }))
//           }
